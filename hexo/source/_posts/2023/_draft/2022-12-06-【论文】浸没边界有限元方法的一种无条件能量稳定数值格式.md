...
浸没边界法广泛应用于心脏力学中的流固耦合计算。为了维持数值求解的稳定性，浸没边界方法对时间步长的限制十分严格，这一点在文献中已经有大量记录[^2010Truncated][^1997Immersed][^2002The][^stockie1999analysis]。2006年，Newren首次发现了浸没边界法无条件稳定格式的构造方法，并给出了几种隐式离散格式。2009年，D. V. Le 等人将JFNK(Jacobian-free Newton–Krylov)方法应用于隐格式的求解，模拟了红细胞在液体中的运动，发现隐格式比显格式快43倍，并发现当固体的刚性增强，隐格式在效率上的优势会更加明显。
...
然而，当前心脏力学的流固耦合模拟仍以显式离散的浸没边界法为主，需要使用非常小的时间步长，例如模拟二尖瓣[]、三尖瓣[]的时间步长在$10^{-6}$这一数量级，模拟左心室[]的时间步长在$10^{-5}$这一数量级，这些都远远小于空间离散的步长。隐格式没有被广泛采用有多方面的原因，首当其冲的是数学模型的变化。

Boffi提出了新的数学框架，不仅可以求解带有厚度的固体，固体和流体的边界满足跨越边界条件，固体可以使用通用的本构关系。在此，Newren提出的理论不再适用。Boffi理论上分析了声称，他们提出的格式是无条件稳定的，但是据我所知，要实现他们。他们自己也没有实现。Boffi等人提出了DLM-IBM方法，也是一种无条件稳定的数值方法，但是它要求，固体的本构关系是线性的，并且这种方法不能套用已有的软件进行求解。

其次，插值算子和延拓算子的对称性有要求。按经验，拉格朗日网格要比欧拉网格密2倍以上才能避免出现这种流体泄漏的现象，尤其是固体会发生大变形的问题。Shankar等人提出过类似的方法。他们使用[[数据点|数据点(data sites)]]和[[样本点(sample sites)]]两个名词，对应本文中的控制点和作用点，他们使用[[径向基函数|径向基函数(radial basis function, RBF)]]代替了[正则化的delta 函数核](正则化的delta%20函数核)。不同的是，本文中的[[插值算子]]和[[延拓算子]]满足离散形式的[[对偶性|对偶性(discrete adjoint property)]]，意味着[[拉格朗日-欧拉相互作用方程|拉格朗日-欧拉相互作用]]的的过程中满足能量守恒性质。
