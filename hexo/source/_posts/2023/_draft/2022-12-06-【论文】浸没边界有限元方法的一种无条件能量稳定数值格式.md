...
本文针对浸没边界/有限元(Immersed Boundary/Finite Element, IB/FE)方法提出了一种无条件稳定的数值离散格式。对于空间离散，流体采用交错网格方法进行离散，固体采用有限元方法进行离散；对于时间离散，采用向后欧拉格式进行离散。如果固体材料的本构关系的应变能函数是关于形变梯度的凸函数，并且插值算子和延拓算子是对称的(self-adjoint)，则采用向后欧拉格式离散的IB/FE 方法是无条件稳定的。针对离散格式所构造的非线性代数系统，采用 Jacobian-free Newton-Krvlov 方法进行求解。通过数值算例验证了格式的有效性和准确性。数值结果表明本文提出的数值离散格式能够在维持数值模拟的准确性的情况下，显著地增加了IB/FE方法在求解流固耦合问题时的时间步长。

<!-- more -->
### 1 引言 Introduction

浸没边界法被广泛应用于心脏力学中的流固耦合计算，但为了确保数值求解的稳定性，该方法对时间步长的限制非常严格，这一点已经有大量文献进行了记录[^2010Truncated][^1997Immersed][^2002The][^stockie1999analysis]。在2006年，Newren首次提出了浸没边界法无条件稳定格式的构造方法，并给出了几种隐式离散格式。随后在2009年，D. V. Le等人将JFNK（Jacobian-free Newton-Krylov）方法应用于隐式格式的求解，成功模拟了红细胞在液体中的运动，并发现隐式格式的计算速度比显式格式快43倍，特别是在固体刚性增强的情况下，隐式格式的效率优势更加明显。吴家阳将这种方法应用到 IB/LB 中，时间步长可以增加到原来的 200 多倍[]。
...
然而，目前心脏力学的流固耦合模拟仍然主要采用显式离散的浸没边界法，需要使用非常小的时间步长。例如，模拟二尖瓣和三尖瓣的时间步长在$10^{-6}$数量级[][]，模拟左心室的时间步长在$10^{-5}$数量级[]，远远小于空间离散的步长。

隐格式没有被广泛采用有多方面的原因，首当其冲的是原始的浸没边界法不能准确地模拟心脏流固耦合问题。Boffi提出了一种新的数学框架，不仅可以求解带有厚度的固体，而且可以处理固体和流体之间的跨越边界条件，并且固体可以使用更加通用的本构关系而不是纤维模型，能够更加精确地描述流固耦合问题。然而，Newren提出的理论在此不再适用。Boffi理论上分析了他们提出的格式是无条件稳定的，但实现起来仍有困难，至今未有人实现。Boffi等人提出了DLM-IBM方法，也是一种无条件稳定的数值方法，但是它要求固体的本构关系是线性的。不能套用现有的软件进行求解。

其次，插值算子和延拓算子的对称性是必须的。根据经验，拉格朗日网格要比欧拉网格密集2倍以上才能避免流体泄漏现象，特别是当固体发生大变形时。Shankar等人提出了类似的方法，使用数据点和样本点来代替本文中的控制点和作用点，并使用径向基函数代替正则化的delta函数核。与此不同的是，本文中的插值算子和延拓算子满足离散形式的对偶性，意味着拉格朗日-欧拉相互作用方程的过程中满足能量守恒性质。

本文的主要创新点是在处理拉格朗日-欧拉相互作用方程的方法上。传统的浸没边界法中，力的延拓算子和速度的插值算子是直接作用于拉格朗日网格节点和笛卡尔网格节点之间。这种方法有一个缺点，即如果拉格朗日网格节点的物理间距大于欧拉网格，则流体和固体接触面处会出现严重的流体泄漏现象，即流线会穿过固体。根据经验，为了避免这种流体泄漏现象，拉格朗日网格要比欧拉网格密集2倍以上，特别是当固体发生大变形时。
